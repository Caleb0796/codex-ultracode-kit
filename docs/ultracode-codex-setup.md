...
**3. `AGENTS.md` (this directory, ready to install)** — adds an "Ultra mode" trigger section plus three always-on rigor rules that published evals show actually bind (search-until-dry sweeps, hunk-by-hunk diff re-read + grep-changed-symbols, requirement-to-diff completeness mapping). Install with:
```bash
cp ~/Desktop/agent_md/AGENTS.md ~/.codex/AGENTS.md
```
...
- **Hooks** — `codex_hooks` is stable in your build: a turn-stop hook can run tests/linters mechanically and feed failures back, enforcing verification instead of trusting prose.
